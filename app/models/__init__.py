...
class Match(Base, AuditMixin):
    ...
    location_id: Mapped[Optional[int]] = mapped_column(ForeignKey("location.id"), nullable=True)
    location: Mapped[Optional["Location"]] = relationship(back_populates="matches", lazy='noload')
    mvp_id: Mapped[Optional[int]] = mapped_column(ForeignKey("player.id"), nullable=True)
